...
use crate::client_connection::DiscoveryClient;
...
impl Client {
    ...
    pub fn start_client(ip_addr: IpAddr) -> Result<Client, Box<dyn Error>> {
       ...
       // drop(discovery_client);  //fondamentale per disconnettere il socket e renderlo cosi possibile da usare per gstreamer
        let mut player = Client::new(client_ip.clone(),client_port)?;
    ...
    fn stop_client(mut player:Client ) -> Result<(), Box<dyn Error>> {
